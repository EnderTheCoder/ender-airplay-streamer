    ...
    if (impl_->config.log_callback) {
        raop_set_log_callback(impl_->raop, [](void* cls, int level, const char* msg) {
            ...
            if (streamer && streamer->impl_->config.log_callback) {
                LogLevel log_level;
                switch(level) {
                    case 0: case 1: case 2: case 3: log_level = LogLevel::Error; break;
                    case 4: log_level = LogLevel::Warning; break;
                    case 5: case 6: log_level = LogLevel::Info; break;
                    case 7: log_level = LogLevel::Debug; break;
